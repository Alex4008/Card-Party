...
public class MultiplayerUnoPlayer {
	...
	boolean turn; //Used to determine if its the players turn
	
	public MultiplayerUnoPlayer(Main m, Client c, int playerID, Hand hand) {
		this.m = m;
		this.c = c;
		...
		if(turn) {
			...
			if(selectingColor == false) {
				...
				if(callUno.getClicked()) {
					...
					//TODO: Tell client to tell server that we challanged uno
				}
			...
			//WAIT - the server SHOULD tell us when its our turn
		}
	}
	...
	private void runHumanTurn() {
		...
		if(firstPriv && selectingColor) {
			...
			if(colorPick.getSelectedColor() != UnoColor.NULL) {
				//TODO: Tell client to tell server that we changed the color
				colorPick.reset();
				firstPriv = false;
				selectingColor = false;
			}
		...
		if(forceStackTwo) {
			...
			if(pass.getClicked()) {
				//TODO: Tell client to tell server that we need to draw a card
				complete = true;
			}
			...
			if(cardBeingPlayed != null) {
				...
				if(cardBeingPlayed.getUnoValue() == UnoValue.DRAWTWO) {
					...
					//TODO: Tell server to add a card to the discard
